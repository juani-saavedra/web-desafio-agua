...
| *(no estaba en la lista original)* Arroyo Toro | Tigre (Primera Sección de Islas) | **PARCIAL — discontinuada** | INA-INTA | Sí (histórico) | 34847 | -58.596, -34.3225 | Resuelta ahora: `alerta.ina.gob.ar/a5/obs/puntual/series?estacion_id=6831` (`cero_ign: -0.266`, 3.675 obs) + confirmado cruzando con `ina.gob.ar/delta/index.php?seccion=9` (mismo `seriesId`). **Corrección post-relevamiento** (ver nota⁵): la serie no reporta desde fines de 2023, no está "activa" |
...
5. **Corrección — Arroyo Toro NO está activa.** Este relevamiento la reportó como "serie pública activa" por tener `cero_ign` y 3.675 observaciones históricas, pero no se chequeó la fecha del último dato. Verificación posterior contra `alerta.ina.gob.ar/a5/obs/puntual/series/34847`: `date_range` es `2023-06-26` a `2023-12-30` — **dejó de reportar hace más de dos años**, no tiene datos en ninguna ventana reciente. Es cobertura histórica, no una estación en servicio hoy. Al agregarla al dataset (`estaciones.config.json`), la ingesta la muestra correctamente como `calidad.estado: "sin_dato"`.
...
- **Nuevo hallazgo no buscado**: Arroyo Toro sí cae dentro de la zona (Primera Sección de Islas, Tigre) y tiene `cero_ign` resuelto — pero **no está activa**: su serie (34847) no reporta desde fines de 2023 (ver nota⁵). No estaba en ningún documento previo del proyecto; es cobertura histórica, no una estación viva.
